...
});
{
  ...
  const uAlgs = ["", "z", "x", "z'", "x'", "x2"].map((s) => parse(s));
  const yAlg = parse("y");
  for (const uAlg of uAlgs) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      puzzleOrientationCache[idxU][idxL] = Invert(Puzzles["3x3x3"], orientationKpuzzle.state);
    }
  }
}
...
function reorientPuzzle(s, idxU, idxL) {
  const orientationTransformation = Invert(Puzzles["3x3x3"], puzzleOrientationCache[idxU][idxL]);
  return Combine(Puzzles["3x3x3"], s, orientationTransformation);
}
...
//# sourceMappingURL=chunk.6USTMHT3.js.map
