...
NPM_COMMANDS = build build-esm build-bundle-global build-types build-bin build-experiments build-solve-worker generate-js generate-js-parsers generate-js-svg dev clean test test-dist-esm-parcel test-dist-experiments test-import-restrictions test-jest test-dist-esm-import test-tsc test-dist-esm-perf format setup lint prepack parcel-build-for-twizzle-net
