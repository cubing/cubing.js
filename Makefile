...
NPM_COMMANDS = build build-esm build-cjs build-bundle-global build-types build-bin build-experiments build-solve-worker generate-js generate-js-parsers generate-js-svg dev clean test test-dist-parcel test-experiments test-import-restrictions test-jest test-node-require test-node-import test-tsc test-dist-perf format setup lint prepack parcel-build-for-twizzle-net
