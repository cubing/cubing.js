  ...
  implements AlgIndexer<P>
{
  ...
  private getMoveWithRange(index: number): AnimLeafWithRange {
    return this.animLeaves[Math.min(index, this.animLeaves.length - 1)];
  }
  ...
  public timestampToPosition(
    ...
    for (const moveWithRange of this.animLeaves.slice(
      0,
      currentMoveInfo.stateIndex,
    )) {
      if (moveWithRange.animLeaf.is(Move)) {
        state = this.puzzle.combine(
          state,
          this.puzzle.stateFromMove(moveWithRange.animLeaf as Move),
        );
      }
    }
  ...
  public currentMoveInfo(timestamp: Timestamp): CurrentMoveInfo {
    ...
    for (const moveWithRange of this.animLeaves) {
      if (moveWithRange.end <= timestamp) {
        ...
        const move: Move | null = moveWithRange.animLeaf.as(Move);
        if (move === null) {
          break;
        }
        const currentMove = {
  ...
  public stateAtIndex(index: number, startTransformation?: State<P>): State<P> {
    ...
    for (let i = 0; i < this.animLeaves.length && i < index; i++) {
      const moveWithRange = this.animLeaves[i];
      const move = moveWithRange.animLeaf.as(Move);
      if (move !== null) {
  ...
  public transformAtIndex(index: number): State<P> {
    ...
    for (const moveWithRange of this.animLeaves.slice(0, index)) {
      const move = moveWithRange.animLeaf.as(Move);
      if (move !== null) {
        state = this.puzzle.combine(state, this.puzzle.stateFromMove(move));
      }
