...
import { reid3x3x3ToTwizzleBinary } from "./binary3x3x3";
...
describe("Binary 3x3x3", () => {
  ...
  it("handles rotations", () => {
    expect(reid3x3x3ToTwizzleBinary(await stateForAlg("z y"))).toEqual(
      new Uint8Array([0, 0, 0, 0, 0, 0, 0, 0, 0, 176, 0]),
    );

    expect(reid3x3x3ToTwizzleBinary(await stateForAlg("x2 z'"))).toEqual(
...
describe("Hex", () => {
  it("conversion works", () => {
    expect(
      bufferToSpacedHex(reid3x3x3ToTwizzleBinary(await stateForAlg(""))),
