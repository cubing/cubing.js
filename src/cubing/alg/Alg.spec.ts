...
describe("Clock", () => {
  it("parses notation", () => {
    ...
    expect(new Alg("UR+").toString()).toEqual("UR1+"); // TODO: Reject UR+. https://github.com/cubing/cubing.js/issues/110
    expect(() => new Alg("UR+2").toString()).toThrow(
      /Unexpected character at index 3. Are you missing a space\?/,
    ); // TODO: Better message
    expect(new Alg("UR_PLUS_4 D_PLUS_3'").toString()).toEqual("UR4+ D3-");
  });
});
...
describe("Megaminx", () => {
  it("parses notation", () => {
    ...
    expect(new Alg("R1++").toString()).toEqual("R++"); // TODO: Reject R1++. https://github.com/cubing/cubing.js/issues/110
    expect(() => new Alg("R2++").toString()).toThrow(
      "++ or -- moves cannot have an amount other than 1",
    );
...
describe("Square-1", () => {
  it("parses notation", () => {
    ...
    expect(new Alg("_SLASH_ (U_SQ_3' D_SQ_0)").toString()).toEqual("/ (-3, 0)");
  });
});
