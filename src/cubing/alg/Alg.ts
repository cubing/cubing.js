      ...
      while (inputStack.length > 0) {
        ...
        if (!isOperator(sign)) {
          ...
          while (
            ...
            operatorLevel(sign) <= operatorLevel(operatorStack.slice(-1))
          ) {
            ...
            for (let j = minj; j <= arr1.length / 2 - 1; j++) {
              ...
              if (arr1[i - 1][0] === arr1[i + j - 1][0]) {
                ...
                for (let ir = minAmount; ir <= maxAmount; ir++) {
                  ...
                  part2x = simplify(
                    invert(part1x).concat(repeatEnd(arr1.slice(0, i + j), jr)),
                  );
                ...
                let commuteCase = [];
                if (
                  arr1[i - 1][0] in commute &&
                  arr1[i + j - 1][0] in commute
                ) {
                  if (
                    commute[arr1[i + j - 1][0]].class ===
                      commute[arr1[i - 1][0]].class &&
                    arr1[i + j - 1][0] !== arr1[i - 1][0]
                  ) {
                    ...
                    if (i >= 2) {
                      if (
                        arr1[i - 1][0] in commute &&
                        arr1[i - 2][0] in commute
                      ) {
                        if (
                          commute[arr1[i - 2][0]].class ===
                          commute[arr1[i - 1][0]].class
                        ) {
                          commuteAddList1.push(part1x);
                          commuteCase = simplify(
                            part2x.concat(arr1.slice(i - 2, i)),
                          );
                          commuteAddList2.push(commuteCase);
                        }
                      }
                ...
                if (arr1[i][0] in commute && arr1[i + j][0] in commute) {
                  if (
                    commute[arr1[i + j][0]].class ===
                      commute[arr1[i][0]].class &&
                    arr1[i + j][0] !== arr1[i][0]
                  ) {
                    // For c R b L c' R' b' L' = [c R b R, R' L c'] = [c R L',L b R]
                    commuteCase = simplify(
                      part1x.concat(invert([arr1[i + j]])),
                    );
                    ...
                    if (arr1.length >= i + j + 2) {
                      if (
                        ...
                        if (
                          ...
                          commuteCase = simplify(
                            part1x.concat(invert(arr1.slice(i + j, i + j + 2))),
                          );
                          commuteAddList1.push(commuteCase);
                          commuteCase = simplify(
                            arr1.slice(i + j, i + j + 2).concat(part2x),
                          );
                          commuteAddList2.push(commuteCase);
                        }
                      }
              ...
              for (const commuteAddKey in commuteAddList1) {
                ...
                const arrb = simplify(
                  part2x.concat(part1x, invert(part2x), invert(part1x), arr1),
                );
                let partb = "";
                if (depth > 1) {
                ...
                for (let k = 2; k <= j; k++) {
                  if (
                    (arr[len - k][0] in commute &&
                      arr[len - (k - 1)][0] in commute) === false
                  ) {
                    canCommute = false;
                    break;
                  }
