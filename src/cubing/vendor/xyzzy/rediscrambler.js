...
let move_L = [permutation_from_cycle([ 0,  1,  4], 12),  unsparsify_list({0: 2}, 8)];
let move_B = [permutation_from_cycle([ 1,  2,  5], 12),  unsparsify_list({1: 2}, 8)];
let move_R = [permutation_from_cycle([ 2,  3,  6], 12),  unsparsify_list({2: 2}, 8)];
let move_F = [permutation_from_cycle([ 3,  0,  7], 12),  unsparsify_list({3: 2}, 8)];
let move_l = [permutation_from_cycle([ 9,  8,  4], 12),  unsparsify_list({4: 2}, 8)];
let move_b = [permutation_from_cycle([10,  9,  5], 12),  unsparsify_list({5: 2}, 8)];
let move_r = [permutation_from_cycle([11, 10,  6], 12),  unsparsify_list({6: 2}, 8)];
let move_f = [permutation_from_cycle([ 8, 11,  7], 12),  unsparsify_list({7: 2}, 8)];
...
{
	let p = index_to_evenpermutation(Math.floor(Math.random() * factorial(12)/2), 12);
	let o = Array(8);
...
function solve(state)
{
	...
	for (let i = 0; i < 25; i++)
	{
		...
		console.log(`to ${new_state} in ${sol1.length} moves; total move count ${sol1.length + sol2.length}`);
...
function generate_phase1_corner_mtable()
{
	...
	{
		mtable[i] = Array(8);
		let o = [i % 3, Math.floor(i/3) % 3, Math.floor(i/9) % 3, Math.floor(i/27)];
...
function generate_phase1_edge_ptable()
{
	...
	return tables.phase1ep = bfs(generate_phase1_edge_mtable(), [8+12*(9+12*(10+12*11))]);
}
