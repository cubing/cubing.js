...
import type { PuzzleBaseShape, PuzzleCutType } from "../../../cubing/puzzle-geometry/PuzzleGeometry";
...
export class TwistyPuzzleDescriptionInput extends HTMLElement {
  ...
  dispatchPuzzleDescription() {
    ...
    for (const cutType of EXPERIMENTAL_PUZZLE_CUT_TYPES) {
      for (const input of Array.from(
        ...
        descriptionStringParts.push(this.clean(this.puzzleShape, cutType, +input.value).toString());
      }
    }
  ...
  clean(base: PuzzleBaseShape, cut: PuzzleCutType, ival: number): number {
    ...
    for (const v of goodpts) {
      if (Math.abs(v - ival) < SnapEpsilon && Math.abs(v - ival) < Math.abs(best - ival)) {
        best = v;
      }
    }
    if (best != 1000) {
...
const KeyPoints: Record<string, number[]> = {
  ...
  tf: [0],
  cv: [0, 0.192450089729875, 0.239059095459766, 0.577350269189594,
       0.915641442663986, 0.962250448649376, 1.15470053837926, 1.34715062810913],
  ce: [0, 0.2357022604, 0.261021839065223, 0.353553390593277, 0.414205290678296,
